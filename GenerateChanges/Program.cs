	...
	class AccessDatabaseExample
	{
		...
		static void Main()
		{
			...
			(string, string)[] tables = [
				("SELECT [All].[PrimaryID] AS [ID], [All].[Player ID], [ALL].[Last Name], [ALL].[First Name], [ALL].[Grade], [ALL].[SchoolCode], [ALL].[Level], [ALL].[Promoted/Demoted], [ALL].[SCATournaments], [ALL].[new], [ALL].[Time], [ALL].[1], [ALL].[2], [ALL].[3], [ALL].[4], [ALL].[5], [ALL].[6], [ALL].[7], [ALL].[JuneUnder], [ALL].[JuneOver], [ALL].[AugUnder], [ALL].[AugOver], [ALL].[Notes], [ALL].[SwimLevel], [ALL].[SwimmingComments], [ALL].[YESphoto], [ALL].[PhotoComments], [ALL].[Extras], [All].[CamperPaid], [All].[Teeshirt], [All].[Discount]\r\nFROM [ALL];", "Camper Info"),
				("SELECT [Camper Info].[Player ID], [Camper Info].[Last Name], [Camper Info].[First Name], [Camp Contact Info].Address, [Camp Contact Info].City, [Camp Contact Info].State, [Camp Contact Info].ZipCode, [Camp Contact Info].HPhone, [Camp Contact Info].WPhone1, [Camp Contact Info].WPhone2, [Camp Contact Info].CPhone, [Camp Contact Info].EMail\r\nFROM [Camp Contact Info] INNER JOIN [Camper Info] ON [Camp Contact Info].[PrimaryID] = [Camper Info].[ID];", "Contact Info"),
				("SELECT [Camper Info].[Player ID], [Camper Info].[Last Name], [Camper Info].[First Name], [Camp Contact Info].Address, [Camp Contact Info].City, [Camp Contact Info].State, [Camp Contact Info].ZipCode, [Camp Contact Info].HPhone, [Camper Info].SchoolCode, [Camper Info].Grade, [Camp Contact Info].EMail\r\nFROM [Camp Contact Info] INNER JOIN [Camper Info] ON [Camp Contact Info].PrimaryID = [Camper Info].ID\r\nWHERE ((([Camper Info].[Player ID])>=60528) OR (([Camper Info].[Player ID])=0))\r\nORDER BY [Camper Info].[Last Name], [Camper Info].[First Name];\r\n", "New Player"),
			];
